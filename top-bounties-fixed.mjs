...
async function showTopBounties() {
  ...
  for (const org of orgs) {
    try {
      ...
      bountiesWithReward.forEach(bounty => {
  ...
  topBounties.forEach((bounty, idx) => {
    ...
    console.log(`   📊 ${bounty.orgName} | ${bounty.kind} | ${bounty.reward_type}`);
  ...
  const orgsFound = [...new Set(allBounties.map(b => b.orgName))];
  ...
  allBounties.forEach(b => {
    orgCounts[b.orgName] = (orgCounts[b.orgName] || 0) + 1;
  });
