...
def test_skill_direct(skill_path, module_name):
    """直接测试 skill 文件是否可导入"""
    try:
...
def main():
    print("=" * 80)
    print("🧪 小红书自动化闭环 - Skill 可用性测试（修复版）")
    print("=" * 80)
    print()
    ...
    if skill_exists:
        success, error = test_skill_direct(
    ...
    if publisher_path.exists():
        success, error = test_skill_direct(
            str(publisher_path / "publisher.py"),
            "publisher"
        )
        print(f"   可导入: {'✅' if success else '❌'}")
        if error:
    ...
    # 统计
    total = len(results)
    available = sum(1 for r in results if "✅" in r["status"] and "⚠️" not in r["status"])
    ...
    # 保存测试报告
    report_path = Path("/home/vimalinx/.openclaw/workspace/xhs-automation-test-report.json")
    import json
    with open(report_path, 'w', encoding='utf-8') as f:
        json.dump({
            "timestamp": "2026-02-20 22:55:00",
            "results": results,
            "summary": {
                "total": total,
                "available": available,
